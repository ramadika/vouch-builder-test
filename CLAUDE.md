...
5. **Minimise creative drift on both Claude calls.** The original intent here was `temperature: 0`. The current model (`claude-opus-4-8`) **removed the `temperature` parameter** — sending it returns a 400 — so temperature can no longer be set. We achieve the same low-drift goal three other ways, and these must not be relaxed without sign-off: (a) the **parse** call uses **structured outputs** (a fixed JSON schema via `output_config.format`), so the model cannot emit fields outside the schema; (b) both calls use grounding-first system prompts; (c) the real reasoning happens deterministically in `reconcile.ts`, not in the model. See `DECISIONS.md` for the rationale. If you switch to a model that supports `temperature`, set it to 0.
